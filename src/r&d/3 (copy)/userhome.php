...
?>
		<div class="box">
			...
			while($ary = mysql_fetch_array($q))
			{
				...
				echo "<tr><td>$linkbefore$product$linkafter</td><td>$owner</td><td>$formatName</td><td>$$price</td><td>$user_share%</td><td>$sales</td><td><a href=\"" . WEBROOT . "stubcode.php?id=$stubID\">Get Codes</a> $facebook</tr>";
			}
		...
		<div class="box">
			...
			while($ary = mysql_fetch_array($q))
			{
				...
				echo "<tr><td>$$price</td><td>$linkbefore$productName$linkafter</td><td>$ownerName</td><td>$formatName</td><td><a href=\"$shareLink\">Share</a>$fb</td></tr>";
			}
			?>
			</table>
			<br />
			...
			<h2>Recommended Stubs</h2>
				...
				<?php
					...
					while($q && $stub = mysql_fetch_array($q))
					{
						...
						$disp = "<table  style=\"width: 100%; \"><tr><td style=\"width:62px\"><img src=\"prodimg.php?id=$productID\" style=\"float:left; width:60px; height:60px;\"/></td><td>$prodName</td><td>$prodDesc</td><td>$stubHTML</td></tr></table>";
						//
