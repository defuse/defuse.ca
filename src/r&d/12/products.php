...
if($q && mysql_num_rows($q) > 0)
{
	while($prod_info = mysql_fetch_array($q))
	{
		...
		$id = (int)($prod_info['id']);
		echo "<tr><td>$name</td><td>$desc</td><td>$price</td><td>$owner_cut</td><td>$sharer_cut</td><td>$charity_cut</td><td>$stubshare_cut</td><td>$sales</td><td><a href=\"" . WEBROOT . "stubcode.php?id=$mainstub\">Get Links</a></td>";
		echo "</tr>";
	}
}
?>
