...
class StubShare
{
	...
	public static function GetUserShareProfit($username)
	{
		$safe_user = mysql_real_escape_string($username);
		$q = mysql_query("SELECT share_profit FROM users WHERE username='$safe_user'");
		if($q && mysql_num_rows($q) >= 1)
		{
			$ary = mysql_fetch_array($q);
			return $ary['share_profit'];
		}
		else
		{
			return -1;
	...
	public static function GetUserCharityContrib($username)
	{
		$safe_user = mysql_real_escape_string($username);
		$q = mysql_query("SELECT charity_contrib FROM users WHERE username='$safe_user'");
		if($q && mysql_num_rows($q) >= 1)
		{
			$ary = mysql_fetch_array($q);
			return $ary['charity_contrib'];
		}
		else
		{
			return -1;
	...
	public static function LimitText($text, $moreurl = "")
	{
		$MAX_SIZE = 50;
		$oldtext = $text;
		$more = "";
		if(strlen($text) > $MAX_SIZE)
		{
			...
			if(!empty($moreurl))
			{
				$more = "<a href=\"$moreurl\">more</a>";
			}
		}
		$oldtext = htmlspecialchars($oldtext);
		$text = htmlspecialchars($text);

		return "<div title=\"$oldtext\">$text&nbsp; $more</div>";
	}
	...
	public static function GetTwitterImageLink($stub_id)
	{
		...
		$link = "<iframe allowtransparency=\"true\" frameborder=\"0\" scrolling=\"no\" src=\"http://platform.twitter.com/widgets/tweet_button.html\" style=\"width:60px; height: 20px;\"></iframe>";
		return $link;
	}
	...
	public static function ProcessPurchase($username, $stub_id)
	{
		...
		if($cost <= $balance)
		{
			...
			//$profit_donate = (double)$userInfo['profit_for_charity']; //TODO: this should be the SHARER'S  profit_for_charity

			$ownerID = (int)$productInfo['owner'];
			$charityID = (int)$userInfo['charity'];
			$sharerID = (int)$stubInfo['owner'];

			$sharerInfo = self::GetUserInfo(self::GetUsernameFromID($sharerID));
			$profit_donate = (double)$sharerInfo['profit_for_charity'];
			...
			$stub_charity_total =  $charity_total;
			$profit_to_charity = $sharer_total * $profit_donate / 100;
			$charity_total += $profit_to_charity;
			$sharer_total -= $profit_to_charity;


			mysql_query("UPDATE users SET balance='$balance', charity_contrib = charity_contrib + $stub_charity_total WHERE id='$userid'");
			...
			mysql_query("UPDATE users SET balance = balance + $sharer_total, share_profit = share_profit + $sharer_total, charity_contrib = charity_contrib + $profit_to_charity WHERE id='$sharerID'");
