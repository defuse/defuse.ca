...
$filetype = sqlsani($_GET['type']);
$safehash = sqlsani($_GET['id']);
$rescan = sqlsani($_GET['rescan']);

if($rescan == '1'){
	$backup = mysql_fetch_array(mysql_query("SELECT * FROM queue WHERE hash='$safehash' AND username='0'"));
	if($backup) {
		$del = mysql_query("DELETE FROM queue WHERE hash='$safehash' AND username='0'");
		AddToQueue($backup['filename'], $backup['filedata'], $backup['arcname'], $backup['arcid'], $backup['hash'], 0, 0);
		mysql_query("UPDATE queue SET complete = '3' WHERE hash = '".$backup['hash']."'");
		header("location:http://localhost/scanner.php?type=file&id=$safehash"); // redirection does not work again.
	} else {
	echo "Error";
	}
die();
...
if($filetype == 'arc'){
$qhash = mysql_query("SELECT * FROM archive WHERE arcid='$safehash'");
while($phash = mysql_fetch_array($qhash)){
if(count($phash)>0){
print "SELECT * FROM queue WHERE hash='".sqlsani($phash['hash'])."'";
$ghash = mysql_fetch_array(mysql_query("SELECT * FROM queue WHERE hash='".sqlsani($phash['hash'])."'"));
if($ghash['complete'] == '4'){
echo "<a href='scanner.php?type=file&id=".$ghash['hash']."&rescan=1'>Rescan File</a> || <a href='scanner.php?type=file&id=".$ghash['hash']."'>Display Result</a>";
} else {
print "<a href='scanner.php?type=file&id=".$ghash['hash']."'>".$ghash['filename']."</a><br />";
}}}
die();
}
...
$sha256 = isdetected($result['sha256']);
