...
if(isset($_GET['id'])) //if they are requiesting the id
{
	...
	if($data['complete'] ==  69) //if the file was JUST uploaded (first view)
	{
		...
		$check = mysql_query("SELECT * FROM queue WHERE sha256 = '$safesha256' AND id != '$safeid' AND (username='$safeuser' OR username='public') AND complete != '69' AND asquared != 'DONTDO' AND avira != 'DONTDO' AND bitdefender != 'DONTDO' AND esafe != 'DONTDO' AND eset != 'DONTDO' AND fprot != 'DONTDO' AND ikarus != 'DONTDO' AND kav != 'DONTDO' AND mcafee != 'DONTDO' AND norton != 'DONTDO' AND panda != 'DONTDO' AND quickheal != 'DONTDO' AND solo != 'DONTDO' AND sophos != 'DONTDO' AND vba32 != 'DONTDO' AND virusbuster != 'DONTDO' ORDER BY id DESC");
...
function Ajaxcmnt(){
Ajaxstop();
var url ="http://ossbox.com/r&d/new/scanres.php?id=<?php echo xss($_GET['id']); ?>";
...
var t =setInterval("Ajaxcmnt()",5000); 
function Ajaxstop(){
var url1 ="http://ossbox.com/r&d/new/scanstop.php?id=<?php echo xss($_GET['id']); ?>";

	try{	
		xmlstop=new XMLHttpRequest();// Firefox, Opera 8.0+, Safari
	}
	catch (e){
		try{
			xmlstop=new ActiveXObject("Msxml2.XMLHTTP"); // Internet Explorer
		}
		catch (e){
		    try{
				xmlstop=new ActiveXObject("Microsoft.XMLHTTP");
			}
			catch (e){
				alert("No AJAX!?");
				return false;
...
xmlstop.onreadystatechange=function(){
	if(xmlstop.readyState==4){
		var m=xmlstop.responseText;
		if(m=="1")
		{
		clearInterval(t);
		}
