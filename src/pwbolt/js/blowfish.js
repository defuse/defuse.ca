...
2010-Mar-10 - the xl_par and xr_par were swapped, so i fixed it and it complies with the test vectors.
NOTE: I made it call encipher twice, just for fun. I can't claim that it will make it more secure, but it is theoretically the same as running it through 32 rounds, without a bigger s box
...
Blowfish.prototype.badrandom=function()
{
return (Math.random()* 4294967296)
};
Blowfish.prototype.randiv=function(){
//This is a shitty random number generator, the security of an IV
var d = new Date();
var randnum = Math.floor(this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + this.badrandom() + d.getTime()) % 4294967296;
return randnum;
};
Blowfish.prototype.encrypt=function(t){
	...
	//use blowfish to improve the quality of the IV
	this.encipher();
	iv1 = this.xl_par;
	iv2 = this.xr_par;
	r = this.wordescape(iv1) + this.wordescape(iv2);
	for(var i=0;i<t.length;i+=16){
		//MODIFIED: the xl_par and xr_par were reversed. - FireXware
		this.xl_par=this.wordunescape(t.substr(i,8));
		this.xr_par=this.wordunescape(t.substr(i+8,8));
		this.xl_par = this.xl_par ^ iv1;
		this.xr_par = this.xr_par ^ iv2;
		this.encipher();
		iv1 = this.xl_par;
		iv2 = this.xr_par;
		r+=this.wordescape(this.xl_par)+this.wordescape(this.xr_par);
	}
	return r;
};
Blowfish.prototype.decrypt=function(t){
	...
	for(var i=0;i<t.length%16;i++) t+='0';
	var r='';
	var iv1 = this.wordunescape(t.substr(0,8));
	var iv2 = this.wordunescape(t.substr(8,8));
var b1 = 0;
var b2 = 0;
	for (var i=16;i<t.length;i+=16){
		//MODIFIED: the xl_par and xr_par were reversed. - FireXware
		this.xl_par=this.wordunescape(t.substr(i,8));
		this.xr_par=this.wordunescape(t.substr(i+8,8));
		b1 = this.xl_par;
