...
<p>
If we run <em>M</em> on a description of itself, <em>M(M)</em>, then what it
will do is start enumerating all possible strings, looking for a valid proof in
<em>T</em> that <em>M</em> never halts when it's run on a description of itself,
and it will halt only when it finds one. Crucially, <em>Prf<sub>T</sub></em>,
the proof-checking algorithm, always halts, so <em>M(M)</em> halts if and only
if there exists a proof within <em>T</em> that <em>M(M)</em> never halts.
</p>
